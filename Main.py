...
# Function to download and save using a requests session
def download_and_save(session, url, save_path):
    try:
...
# Iterate through the list of URLs and download using multithreading
with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
    futures = [executor.submit(download_and_save, session, href, f"downloaded_file_{opened_links.index(href)}") for href in opened_links]
